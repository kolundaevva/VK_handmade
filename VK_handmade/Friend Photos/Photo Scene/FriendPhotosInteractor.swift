...
class FriendPhotosInteractor: FriendPhotosBusinessLogic {
    ...
    func makeRequest(request: FriendPhotos.Model.Request.RequestType) {
        ...
        case .getFriendPhotos(id: let id):
            ...
            API.Client.shared.get(API.Types.Endpoint.getUserPhotos(id: stringId)) { [weak self] (result: Result<API.Types.Response.VKPhoto, API.Types.Error>) in
                switch result {
                case .success(let success):
                    let phts = success.response.items
                    self?.dataManager?.saveUserPhotosData(phts, id: id)
                    self?.presenter?.presentData(response: FriendPhotos.Model.Response.ResponseType.presentFriendPhotos(id: id))
                    
                case .failure(let failure):
                    self?.presenter?.presentData(response: FriendPhotos.Model.Response.ResponseType.presentError(error: failure))
                }
            }
        }
    }
